...
func poison_throw(leader, item):
	...
	if closest_enemy_leader and Behavior.attack.can_hit(leader, closest_enemy_leader):
		...
		Behavior.modifiers.add(closest_enemy_leader, "regen", "poisoned", item.attributes.regen - leader.regen)
	
		closest_enemy_leader.status_effects["poisoned"] = {
			icon = poison_sprite,
			hint = ("Poison, losing %d hp per second.\nMovement speed slowed by %d." %[abs(item.attributes.regen), abs(item.attributes.speed)])
				}

func poison_remove(closest_enemy_leader):
	Behavior.modifiers.remove(closest_enemy_leader, "regen", "poisoned")
